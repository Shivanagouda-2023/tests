Feature: It is to test OD Matrix - Monthly Volumes API
...
Given I set APIGW-Tracking-Header header to ODMatrixMonthlyVolume
And I set bearer token
Given I set time now
When I GET /monthly?date_from=<from>&date_to=<to>&limit=<lim>
...
Then dump request {"URL":"/monthly?date_from=<from>&date_to=<to>&limit=<lim>"}
...
And I set bearer token
Given I set time now
When I GET /monthly?origin_station=<origin>&destination_station=<destination>&limit=<lim>&full_rail_journey=<full_rail_journey>
Given find response time
...
Then dump request {"URL":"/monthly?origin_station=<origin>&destination_station=<destination>&limit=<lim>&full_rail_journey=<full_rail_journey>"}
Then dump response all

Examples:
| origin | destination | lim | full_rail_journey |
| MAN | LIV | 10 | true |
| AAP | BAY | 10 | false |
...
And I set bearer token
Given I set time now
When I GET /monthly?date=<date>&agg=<agg>&limit=10
Given find response time
Then response code should be 200
And response header Content-Type should be application/json
And response body should be valid json
...
Then dump request {"URL":"/monthly?date=<date>&agg=<agg>&limit=10"}
Then dump response all

Examples:
| date | agg |
| 2022-11-01 | weekend_flag, gender, age_band, home_oa_supergroup, purpose, time_period |
