Feature: It is to test OD Matrix - Leg Wise Volumes API
...
Given I set APIGW-Tracking-Header header to ODMatrixLegWiseVolume
And I set bearer token
Given I set time now
When I GET /leg-volumes?date_from=<from>&date_to=<to>&limit=<lim>
...
# And response body path $.pagination_metadata.first_page_full_path should be /v1/origin-destination-matrices/leg-volumes?date_from=<from>&date_to=<to>&limit=<lim>&page=1
And response body path $.pagination_metadata.is_last_page should be true
Then dump request {"URL":"/leg-volumes?date_from=<from>&date_to=<to>&limit=<lim>"}
...
And I set bearer token
Given I set time now
When I GET /leg-volumes?origin_station=<origin>&destination_station=<destination>&agg=<agg>&limit=<lim>
Given find response time
...
Then dump request {"URL":"/leg-volumes?origin_station=<origin>&destination_station=<destination>&agg=<agg>&limit=<lim>"}
Then dump response all
...
And I set bearer token
Given I set time now
When I GET /leg-volumes?date=<date>&agg=<agg>&limit=10
Given find response time
Then response code should be 200
And response header Content-Type should be application/json
And response body should be valid json
...
Then dump request {"URL":"/leg-volumes?date=<date>&agg=<agg>&limit=10"}
Then dump response all

Examples:
| date | agg |
| 2023-05-17 | hour, day_of_week, origin_station_code, origin_station_name, destination_station_code, destination_station_name |
