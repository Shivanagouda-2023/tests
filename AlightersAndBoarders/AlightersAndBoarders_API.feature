Feature: It is to test Alighters and Boarders API
...
Given I set APIGW-Tracking-Header header to AlightersAndBoarders
...
And response body path $.api_name should be alighters-boarders
...
Then dump request {"URL":"?rid=<rid>&limit=10"}
Then dump response all
...
@Negative 
Scenario Outline: TC_04 Error displayed for invalid RID value is passed with the request. 
Given I set APIGW-Tracking-Header header to AlightersAndBoarders
And I set bearer token 
Given I set time now
When I GET ?rid=<rid>&limit=10
...
And response header Content-Type should be application/json
And response body should be valid json
Then response body typed field error should be "Invalid RID Value."
Then dump request {"URL":"?rid=<rid>&limit=10"}
Then dump response all
