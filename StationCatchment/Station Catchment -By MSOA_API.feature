Feature: It is to test Station Catchment - By Local Area (MSOA) API
...
Given I set APIGW-Tracking-Header header to StationCatchmentByMSOA
And I set bearer token
Given I set time now
When I GET /local-areas?date_from=<from>&date_to=<to>&limit=<lim>
...
Then dump request {"URL":"/local-areas?date_from=<from>&date_to=<to>&limit=<lim>"}
...
And I set bearer token
Given I set time now
When I GET /local-areas?station_code=<station_code>&mode=<mode>&agg=<agg>&limit=<lim>
Given find response time
...
Then dump request {"URL":"/local-areas?station_code=<station_code>&mode=<mode>&agg=<agg>&limit=<lim>"}
Then dump response all
...
And I set bearer token
Given I set time now
When I GET /local-areas?date=<date>&agg=<agg>&limit=10
Given find response time
Then response code should be 200
And response header Content-Type should be application/json
And response body should be valid json
...
Then dump request {"URL":"/local-areas?date=<date>&agg=<agg>&limit=10"}
Then dump response all

Examples:
| date | agg |
| 2023-04-01 | weekend_flag, age_band, gender, msoa, msoa_distance, mode_of_transport |