Feature: It is to test Station Catchment Details - By Postal Sector API
...
Given I set APIGW-Tracking-Header header to StationCatchmentByPostalSectors
And I set bearer token
Given I set time now
When I GET /postal-sectors?date_from=<from>&date_to=<to>&limit=<lim>
...
Then dump request {"URL":"/postal-sectors?date_from=<from>&date_to=<to>&limit=<lim>"}
...
And I set bearer token
Given I set time now
When I GET /postal-sectors?station_code=<station_code>&mode=<mode>&agg=<agg>&limit=<lim>
Given find response time
...
Then dump request {"URL":"/postal-sectors?station_code=<station_code>&mode=<mode>&agg=<agg>&limit=<lim>"}
Then dump response all

Examples:
| station_code | mode | agg | lim |
| MAN | RAIL | mode_of_transport | 10 |
| LIV | WALKING | mode_of_transport | 10 |
...
And I set bearer token
Given I set time now
When I GET /postal-sectors?date=<date>&agg=<agg>&limit=10
Given find response time
Then response code should be 200
And response header Content-Type should be application/json
And response body should be valid json
...
Then dump request {"URL":"/postal-sectors?date=<date>&agg=<agg>&limit=10"}
Then dump response all

Examples:
| date | agg |
| 2022-11-01 | weekend_flag, age_band, gender, postal_sector, postal_distance, mode_of_transport |