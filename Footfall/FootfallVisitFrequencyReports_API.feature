Feature: It is to test Visit Frequency-wise Footfall Report API
...
Given I set APIGW-Tracking-Header header to VisitFrequencyFootfallReports
And I set bearer token 
When I GET /visit-frequency?location=<loc>&limit=<lim>
...
Given I set APIGW-Tracking-Header header to VisitFrequencyFootfallReports
And I set bearer token 
When I GET /visit-frequency?date_from=<from>&date_to=<to>&limit=<lim>
Given I Set Timeout for 4500 milliseconds
Then response code should be 200
And response header Content-Type should be application/json
And response body should be valid json
...
Given I set APIGW-Tracking-Header header to VisitFrequencyFootfallReports
And I set bearer token 
When I GET /visit-frequency?date=<date>&limit=<lim>
Given I Set Timeout for 4500 milliseconds
Then response code should be 200
And response header Content-Type should be application/json
And response body should be valid json
...
Given I set APIGW-Tracking-Header header to VisitFrequencyFootfallReports
And I set bearer token 
When I GET /visit-frequency?client_name=<client_name>&limit=<lim>
...
@Positive 
Scenario: TC_04 Successfully retrieves data by its MISC when Oauth token is passed with the request. 
Given I set APIGW-Tracking-Header header to VisitFrequencyFootfallReports
And I set bearer token 
When I GET /visit-frequency?client_name=BT&limit=10&date=2022-08-15&gender=M&age_band=35-44&socio_economic_group=Cosmopolitans
Given I Set Timeout for 4000 milliseconds
Then response code should be 200
And response header Content-Type should be application/json
And response body should be valid json
...
And response body field $.api_name should be of type string
And response body path $.version should be 1.0
And response body path $.data[0].client_name should be BT
And response body path $.data[0].gender should be M
And response body path $.data[0].age_band should be 35-44
And response body path $.data[0].oac_supergroup_name should be Cosmopolitans
Then dump request all
Then dump response all