Feature: It is to test Location Based Footfall Report API
...
Given I set APIGW-Tracking-Header header to LocationBasedFootfallReports
And I set bearer token 
When I GET /generic/location?location=<loc>&limit=<lim>
...
Given I set APIGW-Tracking-Header header to LocationBasedFootfallReports
And I set bearer token 
When I GET /generic/location?date_from=<from>&date_to=<to>&limit=<lim>
Given I Set Timeout for 4500 milliseconds
Then response code should be 200
And response header Content-Type should be application/json
And response body should be valid json
...
Given I set APIGW-Tracking-Header header to LocationBasedFootfallReports
And I set bearer token 
When I GET /generic/location?date=<date>&limit=<lim>
Given I Set Timeout for 4500 milliseconds
Then response code should be 200
And response header Content-Type should be application/json
And response body should be valid json
...
Given I set APIGW-Tracking-Header header to LocationBasedFootfallReports
And I set bearer token 
When I GET /generic/location?date=2022-08-15&
