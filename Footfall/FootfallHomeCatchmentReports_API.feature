Feature: It is to test Home Catchment wise Footfall Reports API
...
Given I set APIGW-Tracking-Header header to HomeCatchmentFootfallReports
And I set bearer token 
When I GET /home-catchment?location=<loc>&limit=<lim>
...
Given I set APIGW-Tracking-Header header to HomeCatchmentFootfallReports
And I set bearer token 
When I GET /home-catchment?date_from=<from>&date_to=<to>&limit=<lim>
Given I Set Timeout for 4500 milliseconds
Then response code should be 200
And response header Content-Type should be application/json
And response body should be valid json
...
Given I set APIGW-Tracking-Header header to HomeCatchmentFootfallReports
And I set bearer token 
When I GET /home-catchment?date=<date>&limit=<lim>
Given I Set Timeout for 4500 milliseconds
Then response code should be 200
And response header Content-Type should be application/json
And response body should be valid json
...
Given I set APIGW-Tracking-Header header to HomeCatchmentFootfallReports
And I set bearer token 
When I GET /home-catchment?client_name=<client_name>&limit=<lim>
