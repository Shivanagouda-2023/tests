Feature: It is to test Transport for London Volumes API
...
Given I set APIGW-Tracking-Header header to TFLVolumes
...
And response body path $.api_name should be transport-for-london-api
...
And I set bearer token
Given I set time now
When I GET ?station_code=<station_code>&mode=<mode>&time_period=<time_period>&limit=<lim>
Given find response time
...
# And response body path $.data[0].time_period should be "<time_period>"
Then dump request {"URL":"?station_code=<station_code>&mode=<mode>&time_period=<time_period>&limit=<lim>"}
Then dump response all

Examples:
| station_code | mode | time_period | lim |
| LST | FERRY | PM_peak | 10 |
| BFR | AIRPLANE | AM_peak | 10 |
...
Examples:
| date | agg |
| 2023-03-05 | weekend_flag, time_period, from_to_station_flag, od_tfl_station |